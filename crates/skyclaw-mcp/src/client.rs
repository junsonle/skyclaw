...
impl McpClient {
    ...
    pub async fn initialize(&self) -> Result<McpServerInfo, SkyclawError> {
    ...
    pub async fn list_tools(&self) -> Result<Vec<McpToolInfo>, SkyclawError> {
        let response = self.transport.send("tools/list", None).await?;
        let result = response.into_result()?;

        let tools_array = result["tools"]
            .as_array()
            .ok_or_else(|| SkyclawError::Tool("MCP tools/list: missing 'tools' array".into()))?;
    ...
    ) -> Result<McpToolResult, SkyclawError> {
        let params = serde_json::json!({
            "name": tool_name,
            "arguments": arguments
        });
    ...
    pub async fn ping(&self) -> Result<(), SkyclawError> {
    ...
    pub async fn close(&self) -> Result<(), SkyclawError> {
