//! MCP configuration — loading and saving `~/.skyclaw/mcp.toml`.
//!
...
/// Path to `~/.skyclaw/mcp.toml`.
pub fn mcp_config_path() -> PathBuf {
    let base = dirs::home_dir().unwrap_or_else(|| PathBuf::from("."));
    base.join(".skyclaw").join("mcp.toml")
}

/// Load MCP config from `~/.skyclaw/mcp.toml`.
...
/// Save MCP config to `~/.skyclaw/mcp.toml`.
