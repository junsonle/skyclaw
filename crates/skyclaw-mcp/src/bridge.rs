//! McpBridgeTool — adapter that wraps an MCP server's tool as a SkyClaw Tool.
//!
//! The agent sees MCP tools identically to built-in tools. The bridge
//! translates between SkyClaw's Tool trait and MCP's JSON-RPC protocol.
...
/// Bridge adapter: wraps a single MCP tool as a SkyClaw Tool.
...
impl Tool for McpBridgeTool {
    ...
    async fn execute(
        ...
        input: ToolInput,
        _ctx: &ToolContext,
    ) -> Result<ToolOutput, skyclaw_core::types::error::SkyclawError> {
