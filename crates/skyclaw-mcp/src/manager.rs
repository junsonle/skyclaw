...
impl McpManager {
    /// Create a new McpManager and load config from `~/.skyclaw/mcp.toml`.
    pub fn new() -> Self {
    ...
    async fn connect_server(&self, config: McpServerConfig) -> Result<usize, SkyclawError> {
        ...
        let transport: Arc<dyn Transport> = match config.transport.as_str() {
            ...
            "http" => Arc::new(HttpTransport::new(
                ...
                config
                    .url
                    .as_deref()
                    .ok_or_else(|| SkyclawError::Tool("HTTP transport requires a URL".into()))?,
                timeout,
                config.headers.clone(),
            )?),
            other => {
                return Err(SkyclawError::Tool(format!("Unknown transport: {}", other)));
            }
        };
    ...
    pub async fn add_server(&self, config: McpServerConfig) -> Result<usize, SkyclawError> {
        config.validate().map_err(SkyclawError::Tool)?;
    ...
    pub async fn remove_server(&self, name: &str) -> Result<(), SkyclawError> {
        let mut servers = self.servers.write().await;
    ...
    pub async fn restart_server(&self, name: &str) -> Result<usize, SkyclawError> {
        let config = {
            let servers = self.servers.read().await;
