...
impl JsonRpcResponse {
    ...
    /// Extract the result value, or return the error as a SkyclawError.
    pub fn into_result(
        self,
    ) -> Result<serde_json::Value, skyclaw_core::types::error::SkyclawError> {
        if let Some(err) = self.error {
            Err(skyclaw_core::types::error::SkyclawError::Tool(format!(
                "MCP error {}: {}",
