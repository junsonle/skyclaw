...
impl TaskGraph {
    ...
    pub fn new(goal: &str, subtasks: Vec<SubTask>) -> Result<Self, SkyclawError> {
        ...
        for task in tasks.values() {
            for dep_id in &task.dependencies {
                if !tasks.contains_key(dep_id) {
                    return Err(SkyclawError::Internal(format!(
    ...
    pub fn mark_running(&mut self, id: &str) -> Result<(), SkyclawError> {
        let task = self.tasks.get_mut(id).ok_or_else(|| {
            SkyclawError::NotFound(format!("Sub-task '{}' not found in graph", id))
        })?;
    ...
    pub fn mark_completed(&mut self, id: &str, result: String) -> Result<(), SkyclawError> {
        let task = self.tasks.get_mut(id).ok_or_else(|| {
            SkyclawError::NotFound(format!("Sub-task '{}' not found in graph", id))
        })?;
    ...
    pub fn mark_failed(&mut self, id: &str, error: String) -> Result<(), SkyclawError> {
        let task = self.tasks.get_mut(id).ok_or_else(|| {
            SkyclawError::NotFound(format!("Sub-task '{}' not found in graph", id))
        })?;
    ...
    pub fn topological_order(&self) -> Result<Vec<String>, SkyclawError> {
    ...
    fn detect_cycle(&self) -> Result<(), SkyclawError> {
        // Build in-degree map
...
fn topological_sort(tasks: &HashMap<String, SubTask>) -> Result<Vec<String>, SkyclawError> {
    let mut in_degree: HashMap<&str, usize> = HashMap::new();
    let mut adj: HashMap<&str, Vec<&str>> = HashMap::new();
...
pub fn parse_decomposition(llm_response: &str, goal: &str) -> Result<Vec<SubTask>, SkyclawError> {
