//! SkyClaw Codex OAuth — OpenAI Codex subscription via OAuth PKCE
//!
//! Enables SkyClaw users to authenticate with their ChatGPT Plus/Pro subscription
...
//! to nothing — zero impact on the rest of SkyClaw.
//!
//! # Modules
//!
...
pub async fn login(headless: bool) -> Result<TokenStore, SkyclawError> {
...
async fn login_browser(
    ...
    state: &str,
) -> Result<TokenStore, SkyclawError> {
...
async fn login_headless(
    ...
    std::io::stdin()
        .read_line(&mut input)
        .map_err(|e| SkyclawError::Auth(format!("Failed to read input: {}", e)))?;

    let input = input.trim();

    // Extract the code and state from the pasted URL
    let url =
        url::Url::parse(input).map_err(|e| SkyclawError::Auth(format!("Invalid URL: {}", e)))?;

    let code = url
        .query_pairs()
        .find(|(k, _)| k == "code")
        .map(|(_, v)| v.to_string())
        .ok_or_else(|| SkyclawError::Auth("No 'code' parameter in URL".to_string()))?;

    let received_state = url
        .query_pairs()
        .find(|(k, _)| k == "state")
        .map(|(_, v)| v.to_string())
        .ok_or_else(|| SkyclawError::Auth("No 'state' parameter in URL".to_string()))?;

    if received_state != state {
        return Err(SkyclawError::Auth(
...
async fn exchange_code(
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| SkyclawError::Auth(format!("Token exchange request failed: {}", e)))?;
    ...
    let token_resp: TokenExchangeResponse = resp
        .json()
        .await
        .map_err(|e| SkyclawError::Auth(format!("Failed to parse token response: {}", e)))?;
...
fn find_available_port() -> Result<u16, SkyclawError> {
    for port in 1455..1555 {
        if std::net::TcpListener::bind(format!("127.0.0.1:{}", port)).is_ok() {
            return Ok(port);
        }
    }
    Err(SkyclawError::Auth(
